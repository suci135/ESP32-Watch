...
String base64Encode(const uint8_t* data, size_t length) {
    ...
    unsigned char* encoded = (unsigned char*)malloc(olen);
...
// Base64编码到缓冲区
void encode_base64(const uint8_t* data, size_t length, unsigned char* output) {
    size_t olen;
    mbedtls_base64_encode(output, length * 2, &olen, data, length);
}

// Base64解码函数
int decode_base64(const unsigned char* input, unsigned char* output) {
    size_t olen;
    int ret = mbedtls_base64_decode(output, 10000, &olen, input, strlen((char*)input));
    return (ret == 0) ? olen : 0;
}
...
#define I2S_OUT_BCLK 12
#define I2S_OUT_LRC 13
#define I2S_OUT_DOUT 11


// MSM261D4030H1CPM config
#define I2S_IN_PORT I2S_NUM_0
// #define I2S_IN_BCLK 4
// #define I2S_IN_LRC 5
// #define I2S_IN_DIN 6

#define PDM_CLK 14   // 连接到 MSM261D4030H1CPM 的时钟引脚
#define PDM_DATA 21  // 连接到 MSM261D4030H1CPM 的数据引脚

// WiFi credentials
const char* ssid = "HONOR 60";
const char* password = "Hi20041021";

// Baidu API credentials
const char* baidu_api_key = "K0jBf4rjmRwbq6mLh33RoxGh";
const char* baidu_secret_key = "dTrqo7SMo6XfMhde3qEstqTpeZPtQGQE";

// Baidu 千帆大模型
char* qianfan_api_key = "ifhBFynUjT9J3BM5GPePnu2c";
char* qianfan_secret_key = "r8vH3rdxunjBblBRtGptnjcKsZkngaIR";
...
//   digitalWrite(LED_BUILTIN, LOW);  //Turn off
  ...
  i2s_pin_config_t pin_config_in = {
    .bck_io_num = -1,      // PDM 不需要 BCK
    .ws_io_num = PDM_CLK,  // PDM 时钟信号
    .data_out_num = -1,
    .data_in_num = PDM_DATA  // PDM 数据输入
  };
...
void baiduTTS_Send(String access_token, String text) {
  ...
  // 进行 URL 编码
  String encodedText = urlEncode(urlEncode(text.c_str()).c_str());
